 ...
 */
class N4GateProcess2 extends GroovyApi {
					...
					try{
						...
						gateXMLBuilder.'gate'() {
									'create-truck-visit'() {
										...
										'truck-visit'('gos-tv-key':gosTvGkey)
									}
						}
							...
							if(noError){
								switch(appt.getTranSubTypeEnum()){
									case TranSubTypeEnum.DM://提空箱预约
										...
										try{
											noError = rm.submitTransaction(trkTrans, fields, null, gate, null, null)
						...
						if(apptsNoDM.size()+cntrList.size()>0){//有DM之外的业务
							...
							gateXMLBuilder.'gate'() {
										'submit-multiple-transactions'() {
											'gate-id'(gateId)
											'stage-id'(this.stageIdIngate)
											'truck-visit'('gos-tv-key':this.gosTvGkey)
											'truck-transactions'{
												if(apptsNoDM.size()>0){
													apptsNoDM.each{appt->
							...
							api.log("GateXML:\r\n" + gateXMLString)
							GenericInvokeResponseWsType gateResponse = argoServicePort.genericInvoke(scope, gateXMLString)
						}
						else{
