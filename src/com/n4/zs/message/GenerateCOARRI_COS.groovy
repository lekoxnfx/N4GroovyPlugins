...
class GenerateCOARRI_COS extends GroovyApi {
	public String fileDir = "D:\\MESSAGE\\COARRI_COS"
	public String fileDirBak = "D:\\MESSAGE\\Backup\\COARRI_COS"
	...
	public String generateCOARRIText(VesselVisitDetails vvd){
		try{
			...
			sql.eachRow(unitQueryStr) {r->
				...
				//只统计COS的箱子
				if(ufv.getUfvUnit().getUnitLineOperator().getBzuId().equals("COS")){
					list_ufv.add(ufv)
				}
			}
			...
			switch(zx){
				case "Z"://出口装船
				voyage = vvd.getVvdObVygNbr() + "E"
				break
				case "X"://进口卸船
				voyage = vvd.getVvdIbVygNbr() + "I"
				break
				default:
			...
			list_ufv.each {u->
				...
				String edi50 = """
				50:${ctn_no}:${ctn_size_type}:${ctn_operator_code}::${ctn_status}:${bl_no}:${seal_no}:${stowage_location}:::'
				""" 
