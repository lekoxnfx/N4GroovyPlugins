 ...
 */
/*
 <groovy class-location="database" class-name="ProcessMT0102">
	 <parameters>
    ...
    public String execute(Map inParameters) {
        ...
        try{
                ...
                if(unitNbr==null){
                    ...
                    if(unitNbr.equals(blNbr)){
                        ...
                        if(matchedBl == null){
                            ...
                            String note = "提单号"+blNbr+"找不到匹配的提单（船舶代码:航次）:" + shipUNCode+":"+jouneyId
                            api.logWarn(note)
                            logList.add(note)
                            unitNoError = false
                            noError = false
                            ...
                            try{
                                ...
                                String note = "解析当前指令时间失败,报文解析出错,时间值:"+ sendTimeStr
                                api.log(note)
                                logList.add(note)
                                timeCorrect = false
                                unitNoError = false
                            ...
                            if(unitNoError){
                                ...
                                try{
                                    ...
                                    if(oriInsDate<=curInsDate){
                                        ...
                                        String note = "海关指令：箱号"+unitNbr+"当前指令比旧指令早，将不会更新"
                                        api.log(note)
                                        logList.add(note)
                                        timeCorrect = false
                                    }
                        ...
                        if(count==1){
                            ...
                            if((unit.getUnitCategory().equals(UnitCategoryEnum.EXPORT)||unit.getUnitCategory().equals(UnitCategoryEnum.TRANSSHIP)&&ieBizCode.equals("E"))){
                                ...
                                String note  = "箱号"+unitNbr+"不是进口或者出口/中转"
                                logList.add(note)
                                unitNoError = false
                                noError=false
                                isIETUnit = false
                                ...
                                if(unitNoError){
                                    ...
                                    if(timeCorrect){
                                        ...
                                        if(jouneyId.equalsIgnoreCase(voyage)&&vesCodeMatch){//比对航次
                                            ...
                                            String note = "海关指令："+"箱号"+unitNbr+"对应船名航次不匹配，收到：" + shipUNCode+":"+jouneyId+",系统："+vesselCode+":"+voyage
                                            api.log(note)
                                            logList.add(note)
                                            noError = false
                                            unitNoError = false
                            ...
                            String note = "海关指令："+"箱号"+unitNbr+"有"+count+"个活动记录！"
                            api.logWarn(note)
                            logList.add(note)
                            unitNoError = false
                            noError = false
            ...
            if (noError){
                ...
                return generateResult(this.ERROR,logList.toString())
            }
        }
